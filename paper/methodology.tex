...
Conversion to (ε, δ)-DP: ε = R\_α + log(1/δ)/(α−1). This follows the
Rényi-DP accounting perspective introduced by Mironov (2017) and the
practical subsampled accounting line developed by Wang et al.~(2019).
The \texttt{RDPAccountant} class tracks cumulative budget per session;
calls are hard-refused once the cap is reached
...
\texttt{model.generate}; model selection follows the published Gemma 4
model card.
